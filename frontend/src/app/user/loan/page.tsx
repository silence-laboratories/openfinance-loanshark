  ...
  return detailsJSON ? (
    <div className="flex flex-col w-screen h-screen justify-center items-center">
      <div className="flex flex-col min-w-80 w-2/5 h-screen mt-10">
        <div className="text-2xl font-bold">Secure Loans Loans</div>
        <div className="text-xs mt-2 text-slate-500">
          Get instant loans upto INR 50,00,000
        </div>
        <div className="border-b-2 mt-4 mb-4" />
        <div>Borrower Name</div>
        <div>{detailsJSON?.name}</div>
        <div className="mt-8">Phone Number</div>
        <div>{detailsJSON?.phone}</div>
        <div className="text-xs mt-2 text-slate-500">
          Enter 10 digit mobile number
        </div>

        <div className="mt-8">Amount</div>
        <div>{detailsJSON?.amount}</div>
        <div className="mt-8">Purpose</div>
        <div>{detailsJSON?.purpose}</div>

        <div className="flex space-x-4 mt-8">
          <div className="text-sm text-white bg-approve py-2 px-8 rounded-lg cursor-pointer">
            {detailsJSON?.loanStatus}
          </div>
        </div>
      </div>
    </div>
